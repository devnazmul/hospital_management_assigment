...
export default function LandLordForm({ handlePopupClose }) {
        ...
        .catch((error) => {
          ...
          toast.custom((t) => (
            <CustomToaster
              t={t}
              type={'error'}
              text={`ID: #00174 - ${error?.response?.data?.message}`}
            />
          ));
        });
    } else {
  ...
  return (
    <div className="border-y-2 py-5">
      <h1 className="my-5 text-base-100 font-bold text-xl text-center">
        Add New Landlord
      </h1>
      <form className=" bg-primary mt-1 rounded-md flex text-base-100 flex-col gap-5">
        ...
        {/* EMAIL  */}
        <div className={`w-full flex flex-col items-center justify-start`}>
          <label className="flex w-full font-semibold">Postcode </label>
          <div className="w-full flex flex-col justify-center">
            <input
              required
              onChange={onChangeFormData}
        ...
        <div className={`flex justify-end items-center gap-5 my-5`}>
          ...
          <button
            ...
            className={`transition-all duration-150 hover:scale-90 border-2 px-5 w-42 py-1 rounded-full border-base-100 bg-base-100 text-primary shadow-md shadow-base-100 flex justify-center items-center`}>
            {isLoading ? (
