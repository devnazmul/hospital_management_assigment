...
export default function UserList({isUbmenu=false}) {
      ...
      .then((res) => {
        ...
        setData(
          res?.data.map((doctor) => ({
            ...
            name: doctor?.name,
            email: doctor?.email,
            role: <span className='px-3 py-2 rounded-xl bg-pink-700 bg-opacity-20 text-pink-700'>{`${doctor?.role.charAt(0).toUpperCase()}${doctor.role.slice(1)
              .toLowerCase()}`}</span>,
            profile: <img className='w-12 h-12 object-cover rounded-xl' src={doctor?.profile_pic_url} alt="" />,
          }))
        );
